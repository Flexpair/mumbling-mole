...
import { UserChannelManager } from "./managers/UserChannelManager";
...
class GlobalBindings {
  constructor(config) {
    ...
    this.audioManager = new AudioManager({
      ...
      voiceHandlerReady: this.voiceHandlerReady,
      micPermissionDenied: this.micPermissionDenied,
      micPermissionErrorMessage: this.micPermissionErrorMessage,
      connected: () => this.thisUser() != null
    });
    
    this.connectionManager = new ConnectionManager({
      connector: this.connector,
      settings: this.settings,
      isLoopbackMode: this.isLoopbackMode,
    ...
    this.userChannelManager = new UserChannelManager({
      getAudioContext: () => this.audioContext || this.audioManager.audioContext,
      selfDeaf: this.selfDeaf,
      userContextMenu: () => this.userContextMenu,
      channelContextMenu: () => this.channelContextMenu,
      requestUnmute: (user) => this.requestUnmute(user),
      requestMute: (user) => this.requestMute(user),
      requestUndeaf: (user) => this.requestUndeaf(user),
      requestDeaf: (user) => this.requestDeaf(user),
      thisUser: this.thisUser,
      root: this.root
    });
    ...
    // Delegate audio methods to AudioManager
    this.startBeep = () => this.audioManager.startBeep();
    this.stopBeep = () => this.audioManager.stopBeep();
    ...
    this._checkFullBeepReadiness = () => this.audioManager._checkFullBeepReadiness();
    this._initializePersistentBeeper = () => this.audioManager._initializePersistentBeeper();
    ...
    this.initializeAudioContext = async () => {
      ...
      // Also maintain reference in GlobalBindings for backward compatibility
    ...
    this._newUser = (user) => {
      this.userChannelManager._newUser(user, compareUsers, userToState, openContextMenu);
    };

    this._newChannel = (channel) => {
      this.userChannelManager._newChannel(channel, compareChannels, openContextMenu);
    };

    this.resetClient = () => {
      this.audioManager.reset();
      this.connectionManager.resetClient(() => this.stopBeep());
    ...
    this._updateLinks = () => {
      this.userChannelManager._updateLinks();
    };
