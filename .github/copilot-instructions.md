...
2. **We ARE in a dev container** - never suggest `docker-compose` or `docker ps` commands
3. **AudioContext singleton** - always use `ensureAudioContext()` from `audio-context-manager.js`
