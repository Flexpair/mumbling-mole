...
// websockify erwartet standardmäßig den Pfad "/websockify" für WS-Upgrades,
// wenn gleichzeitig ein Web-Root (--web) bedient wird. Pfad ist konfigurierbar.
const WS_PATH = process.env.E2E_WS_PATH || process.env.WS_PATH || '/websockify';
...
async function main() {
  ...
  const wsUrl = `ws://${CLIENT_HOST}:${WS_PORT}${WS_PATH.startsWith('/') ? WS_PATH : '/' + WS_PATH}`;
