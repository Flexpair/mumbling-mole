...
class GlobalBindings {
  constructor(config) {
    ...
    this.client = null;
  // Lazy-load netlify-identity-widget to keep initial bundle smaller
  this.netlifyIdentity = null;
    ...
    this.logoutUser = async () => {
      if (!this.netlifyIdentity) {
        const mod = await import(/* webpackChunkName: "netlify-identity-widget" */ "netlify-identity-widget");
        this.netlifyIdentity = mod.default || mod;
      }
    ...
    this.connect = (
      ...
      var ensureIdentityLoaded = async () => {
        if (!this.netlifyIdentity) {
          const mod = await import(/* webpackChunkName: "netlify-identity-widget" */ "netlify-identity-widget");
          this.netlifyIdentity = mod.default || mod;
        }
      };
      // Ensure identity widget is present before using
      // Note: connect can be called from UI; we synchronously gate on identity availability
      // by awaiting the dynamic import at first invocation.
