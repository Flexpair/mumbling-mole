...
class MumbleClient extends EventEmitter {
  ...
  constructor(options) {
    ...
    this._voiceDecoder.on('unknown_codec', codecId => this.emit('unknown_codec', codecId));
    this._data.on('end', this.disconnect.bind(this));
    this._registerErrorHandler(this._data, this._voice, this._dataEncoder, this._dataDecoder, this._voiceEncoder, this._voiceDecoder);
  ...
  createVoiceStream(target = 0, numberOfChannels = 1) {
    ...
    const voiceStream = through2.obj((chunk, encoding, callback) => {
      ...
      const samples = this._samplesPerPacket || chunk.pcm.length / numberOfChannels;
      chunk.bitrate = this.getActualBitrate(samples, chunk.position != null);
      callback(null, chunk);
    });
    const codec = 'Opus'; // TODO
    let seqNum = 0;
    voiceStream.pipe(this._codecs.createEncoderStream(codec)).on('data', data => {
  ...
  _onTextMessage(payload) {
    this.emit('message', this._userById[payload.actor], payload.message, payload.session.map(id => this._userById[id]), payload.channel_id.map(id => this._channelById[id]), payload.tree_id.map(id => this._channelById[id]));
  }
  ...
  getActualBitrate(samplesPerPacket, sendPosition) {
    ...
    const bandwidth = MumbleClient.calcEnforcableBandwidth(bitrate, samplesPerPacket, sendPosition);
  ...
  getMaxBitrate(samplesPerPacket, sendPosition) {
    const overhead = MumbleClient.calcEnforcableBandwidth(0, samplesPerPacket, sendPosition);
